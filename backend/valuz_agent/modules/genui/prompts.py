...
TOOL_DESCRIPTION = (
    ...
    "listed with a query data contract, pass `component_data` containing only "
    ...
    "the component-owned named dataRefs. Components without a query contract receive "
    "typed revision-fixed inline props and do not refresh. Pass params with the "
    ...
    "only the selected schemas to the UI compiler. Do not query a query "
    "component merely to pre-fill `data`, including after a validation failure; "
