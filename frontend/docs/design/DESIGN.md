...
- **加载态三档（v2.7）**：加载指示只有三种形态，由「在哪里等」决定，调用方只选档位和文案，不选尺寸、颜色、位置——
  ①**页面 / 面板首屏** `LoadingState variant="page"`（`PageLoader` 同此）：贴内容区顶部（水平居中、上下留 64px，不垂直居中、不随容器拉伸，保证各页面各面板 spinner 出现在同一高度，也就是内容将要出现的位置），24px spinner `ink-muted`，可选一行 `text-xs ink-meta` 文案在下方；
  ...
  禁止：加载态用品牌紫；手写 `Loader2 className="… animate-spin"`；纯文字加载块；页面首屏把 spinner 顶在左上角或垂直居中到整页中央；同一页两种尺寸。
