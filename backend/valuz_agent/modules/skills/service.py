...
class SkillLibraryService:
    ...
    def _render_manifest(
        ...
        *,
        ...
        The name is sanitized here — this is the single choke point every
        manifest write (create / update / copy / import) passes through, and
        the frontmatter name becomes a *directory* name when the kernel
        materializes the skill into a session's cwd. A name carrying e.g. a
        colon (``react:components``) is uncreatable on Windows.
        """
        ...
        # A body that IS a manifest means the caller read a SKILL.md and handed
        # the whole thing back. Wrapping it buries that frontmatter where no
        # reader looks (they all take the FIRST block) and the real description
        # is gone for good — exactly how ``browser``, ``skill-creator`` and
        # ``valuz-handbook`` shipped with ``description: "---"``. Unwrap and
        # recover instead, so a round-trip that used to corrupt now heals.
        block, rest = split_frontmatter(body)
        carried = parse_frontmatter_mapping(block) if block is not None else None
        if carried is not None and ("name" in carried or "description" in carried):
            if is_placeholder_description(description):
                description = str(carried.get("description") or "").strip()
            body = rest.strip()

        # ``"---"`` is what a frontmatter read returns when it falls through to
        # the delimiter. Never persist it: the write is destructive and the next
        # reader has no way to tell the placeholder from an authored value.
        if is_placeholder_description(description):
            from valuz_agent.modules.skills.errors import SkillManifestDescriptionInvalid

            raise SkillManifestDescriptionInvalid()

        if not body.startswith("#"):
            body = f"# {name}\n\n{body}"
        # Quoted via YAML rather than by hand: a description carrying a quote
        # used to emit a manifest that no longer parses.
        rendered_description = yaml.safe_dump(
            {"description": description}, allow_unicode=True, width=10**6
        ).strip()
        return (
