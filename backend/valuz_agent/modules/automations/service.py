"""AutomationService — CRUD + workspace/agent resolution.
...
- The action is identified by ``(workspace_id, agent_slug)``. We don't carry
...
- ``library_agent`` payloads are normalised at create time:
  ...
  into a chat workspace (existing or lazy-created), and the row stores the
  workspace-local member slug. The row's ``agent_kind`` is preserved
  verbatim so the UI can render a "Library agent" badge — the runner /
  resolver only ever look up by ``(workspace_id, agent_slug)``.
"""
...
class AutomationService:
    ...
    async def _get_workspace_info(self, workspace_id: str) -> tuple[str, str]:
        if self._ws is None:
            raise AutomationWorkspaceNotFound()
        try:
            ws = await self._ws.get_workspace(workspace_id)
            return ws.name, ws.kind
        except AutomationWorkspaceNotFound:
            raise
        except Exception as exc:  # noqa: BLE001 — surface as a clean 404
            raise AutomationWorkspaceNotFound() from exc
    ...
    async def _resolve_agent_name(self, row: AutomationRow) -> str | None:
        ...
        """
        member = await self._members.get(row.workspace_id, row.agent_slug)
        if member is None:
            return None
        try:
    ...
    async def _row_to_item(self, row: AutomationRow) -> AutomationItemResponse:
        ws_name, ws_kind = await self._get_workspace_info(row.workspace_id)
    ...
    async def list_workspace_targets(self) -> list[AutomationWorkspaceTarget]:
        """Workspaces eligible to host a new automation.

        - A fixed "Chat" sentinel (``workspace_id=None``) at the top —
          submitting with this entry tells ``create`` to lazy-create a
          fresh chat workspace named after the automation.
        - Every project-kind workspace, by stable order.

        Chat-kind workspaces are intentionally excluded — they're
        ...
        if self._ws is None:
            return targets
        for ws in await self._ws.list_workspaces():
            if ws.kind == "project":
                targets.append(
                    AutomationWorkspaceTarget(
                        id=ws.id,
    ...
    async def list_automations_in_workspace(
        self, workspace_id: str
    ) -> list[AutomationItemResponse]:
        return [await self._row_to_item(r) for r in await self._ds.list_automations(workspace_id)]

    async def list_all_automations(self) -> list[AutomationItemResponse]:
        return [
            await self._row_to_item(r) for r in await self._ds.list_automations(workspace_id=None)
        ]
    ...
    async def list_automation_groups(
        ...
        Filtered view (project panel): one group per matching workspace,
        ...
        workspace backs them — each chat automation still has its own
        workspace_id (preserves runtime isolation), and the grouping is
        purely a display rule.
        """
        rows = await self._ds.list_automations(workspace_id)
        if workspace_id is not None:
            groups: dict[str, list[AutomationItemResponse]] = {}
            for row in rows:
                groups.setdefault(row.workspace_id, []).append(await self._row_to_item(row))
            result: list[AutomationGroupResponse] = []
            for ws_id, items in groups.items():
                ws_name, ws_kind = await self._get_workspace_info(ws_id)
                result.append(
                    AutomationGroupResponse(
                        workspace_id=ws_id,
                        workspace_name=ws_name,
        ...
        for row in rows:
            try:
                _, ws_kind = await self._get_workspace_info(row.workspace_id)
            except AutomationWorkspaceNotFound:
                # Workspace was deleted out from under the row — drop from
            ...
            if ws_kind == "chat":
                ...
                project_groups.setdefault(row.workspace_id, []).append(item)

        result = []
        if chat_items:
            # Virtual group sentinel — ``workspace_id="chat"`` is a stable
    ...
    # ── Workspace + agent resolution at create time ───────────────────

    async def _resolve_workspace_and_agent(
        self,
        payload: AutomationCreatePayload,
        *,
        calling_session_workspace_id: str | None,
    ) -> tuple[str, str]:
        """Resolve ``(workspace_id, agent_slug)`` for the row.

        Returns the stored ``(workspace_id, agent_slug)`` after applying
        the four routing rules from ADR-021 §4 + §1:

        - chat + ``project_member``  → bind to the caller's chat workspace
          (auto-discovered from ``calling_session_workspace_id``) or fall
          back to lazy-creating one when no calling session is present.
          The slug must already be a member of the resolved workspace.
        - chat + ``library_agent``  → either use the calling chat
          workspace OR lazy-create one, then
          ``AgentService.deploy_agent`` to materialise the
          library agent as a member of that workspace. The stored
          ``agent_slug`` is the new member slug, not the library slug.
        - project + ``project_member`` → bind to the named project
          workspace; verify the slug is a member.
        ...
        """
        # ── Chat path ───────────────────────────────────────────────────
        if payload.workspace_kind == "chat":
            if self._ws is None:
                raise AutomationWorkspaceNotFound()

            # 1. Resolve / lazy-create the workspace
            if payload.workspace_id is not None:
                # MCP-from-chat: caller passed its current chat ws explicitly.
                _, ws_kind = await self._get_workspace_info(payload.workspace_id)
                if ws_kind != "chat":
                    raise AutomationWorkspaceNotFound()
                workspace_id = payload.workspace_id
            elif calling_session_workspace_id is not None:
                # MCP-from-chat fallback: caller didn't pass workspace_id
                # but we know which chat ws they're in. Use it.
                _, ws_kind = await self._get_workspace_info(calling_session_workspace_id)
                if ws_kind == "chat":
                    workspace_id = calling_session_workspace_id
                else:
                    # The calling session was a project session that asked
                    # for kind=chat — treat as "lazy create".
                    fresh = await self._ws.create_chat_workspace_for_session(
                ...
                fresh = await self._ws.create_chat_workspace_for_session(name=payload.name.strip())
                workspace_id = fresh.id

            # 2. Resolve the agent for that workspace
            if payload.agent_kind == "project_member":
                member = await self._members.get(workspace_id, payload.agent_slug)
                if member is None:
                    raise AgentNotInWorkspace()
                return workspace_id, payload.agent_slug

            # ``library_agent`` — instantiate the library agent into this
            # chat workspace. The runner / display then treat it like any
            # other project member.
            if self._agent_svc is None:
                raise RuntimeError(
                    "AutomationService is missing AgentService — required to "
                    "instantiate library agents into chat workspaces"
                )
            source = await self._agents.get_agent(payload.agent_slug)
            if source is None:
                raise AgentNotFound()
            # Derive a workspace-local slug. We hash a short prefix on so the
            # user can have N automations sharing the same source agent in
            # the same workspace without slug collisions.
            instance_slug = f"{payload.agent_slug}-{uuid4().hex[:8]}"
            # ``dedupe=False``: each automation gets its own member handle even
            # when several reference the same source agent in this workspace.
            ...
            return workspace_id, instance_slug
        ...
        member = await self._members.get(payload.workspace_id, payload.agent_slug)
        if member is None:
            raise AgentNotInWorkspace()
        return payload.workspace_id, payload.agent_slug
    ...
    async def create(
        ...
        payload: AutomationCreatePayload,
        *,
        calling_session_workspace_id: str | None = None,
    ) -> AutomationDetailResponse:
        """Create a new automation row.

        ``calling_session_workspace_id`` is the workspace of the kernel
        session that's making the call — relevant only when the
        ``automation`` MCP tool is invoked from inside a chat session, so
        the row binds to that chat's workspace (per the user's "工具创建
        以当前 chat workspace 为准" decision). HTTP create requests pass
        ...
        # Task mode only valid for project workspaces — chat workspaces
        # don't have the multi-member context the project task protocol
        # needs (the kickoff would create an orphan task).
        if payload.action_kind == "task" and payload.workspace_kind != "project":
            raise AutomationTaskOnlyOnProject()

        self._validate_trigger(payload.trigger)

        workspace_id, agent_slug = await self._resolve_workspace_and_agent(
            payload,
    ...
    async def update(
        ...
        if payload.agent_slug is not None:
            ...
            # workspace, regardless of how the row was originally created.
            new_slug = payload.agent_slug.strip()
            if not new_slug:
                raise AutomationAgentRequired()
            member = await self._members.get(row.workspace_id, new_slug)
            if member is None:
                raise AgentNotInWorkspace()
        ...
        if payload.action_kind is not None:
            ...
            # workspace kind from the live workspace row rather than
            # trusting any cached value — workspaces don't change kind
