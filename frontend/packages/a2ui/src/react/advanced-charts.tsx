...
const colors = Array.from({ length: 8 }, (_, index) => `var(--va2-chart-${index + 1})`);
...
function Frame({ title, description, height = 280, weight, accessibility, children }: { title?: unknown; description?: unknown; height?: number; weight?: unknown; accessibility?: unknown; children: ReactNode }) {
  return <figure className="va2-chart va2-pro-chart" style={weightStyle(weight)} {...accessibilityProps(accessibility)}>{(asString(title) || asString(description)) && <figcaption>{asString(title) && <strong>{asString(title)}</strong>}{asString(description) && <span>{asString(description)}</span>}</figcaption>}<div className="va2-chart__canvas" style={{ height }}>{children}</div></figure>;
}

function EmptyPlot() { return <div className="va2-pro-chart__empty">No numerical data</div>; }

export const TimeSeriesChart = createComponentImplementation(TimeSeriesChartApi, ({ props }) => {
  ...
  return <Frame title={props.title} description={props.description} height={props.height ?? 300} weight={props.weight} accessibility={props.accessibility}>{data.length === 0 ? <EmptyPlot/> : <ResponsiveContainer width="100%" height="100%" initialDimension={{ width: 640, height: props.height ?? 300 }}><LineChart data={data} margin={{ top: 10, right: 14, left: -10, bottom: 0 }}><CartesianGrid vertical={false} stroke="var(--va2-chart-grid)" strokeDasharray="3 3"/>{props.showAxes !== false && <XAxis dataKey={props.xKey} axisLine={false} tickLine={false} tick={axisStyle}/>} {props.showAxes !== false && <YAxis axisLine={false} tickLine={false} tick={axisStyle}/>}<Tooltip contentStyle={{ background: "var(--va2-bg)", border: "1px solid var(--va2-border)", borderRadius: "var(--va2-radius-lg)", fontSize: 11 }}/>{props.showLegend !== false && <Legend iconSize={8} wrapperStyle={{ fontSize: 11 }}/>} {typeof props.referenceValue === "number" && <ReferenceLine y={props.referenceValue} stroke="var(--va2-text-disabled)" strokeDasharray="4 4"/>}{(props.series ?? []).map((series, index) => <Line key={series.key} dataKey={series.key} name={asString(series.label, series.key)} type={series.curve ?? "monotone"} stroke={series.color ?? colors[index % colors.length]} strokeWidth={2} dot={false} activeDot={{ r: 3 }} isAnimationActive={false}/>)}</LineChart></ResponsiveContainer>}</Frame>;
});

export const CandlestickChart = createComponentImplementation(CandlestickChartApi, ({ props }) => {
...
export const WaterfallChart = createComponentImplementation(WaterfallChartApi, ({ props }) => {
  ...
  return <Frame title={props.title} description={props.description} height={props.height ?? 280} weight={props.weight} accessibility={props.accessibility}>{data.length === 0 ? <EmptyPlot/> : <svg className="va2-pro-chart__svg" viewBox="0 0 720 250">{points.map((point, index) => { const slot = 660 / points.length; const x = 44 + index * slot; const y1 = scale(point.start, min, max, 216, 18); const y2 = scale(point.end, min, max, 216, 18); const positive = point.value >= 0; return <g key={index} data-kind={point.total ? "total" : positive ? "positive" : "negative"}><rect x={x + slot * .16} y={Math.min(y1, y2)} width={slot * .68} height={Math.max(2, Math.abs(y2 - y1))} rx="3" className="va2-waterfall__bar"/>{index < points.length - 1 && <line x1={x + slot * .84} y1={y2} x2={x + slot * 1.16} y2={y2} className="va2-waterfall__bridge"/>}{props.showValues !== false && <text x={x + slot / 2} y={Math.min(y1, y2) - 5} textAnchor="middle" className="va2-pro-chart__value">{point.value > 0 ? "+" : ""}{point.value}</text>}<text x={x + slot / 2} y="238" textAnchor="middle" className="va2-pro-chart__label">{asString(point.item[props.nameKey])}</text></g>; })}<line x1="38" y1={scale(0, min, max, 216, 18)} x2="710" y2={scale(0, min, max, 216, 18)} className="va2-pro-chart__axis"/></svg>}</Frame>;
});

export const RangeChart = createComponentImplementation(RangeChartApi, ({ props }) => {
...
export const HistogramChart = createComponentImplementation(HistogramChartApi, ({ props }) => {
...
export const BoxPlotChart = createComponentImplementation(BoxPlotChartApi, ({ props }) => {
...
export const BulletChart = createComponentImplementation(BulletChartApi, ({ props }) => {
  const data = asRecords(props.data);
...
export const CalendarHeatmapChart = createComponentImplementation(CalendarHeatmapChartApi, ({ props }) => {
...
export const NetworkGraph = createComponentImplementation(NetworkGraphApi, ({ props }) => {
