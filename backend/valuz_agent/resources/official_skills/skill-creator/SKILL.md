...
The skill creator is liable to be used by people across a wide range of familiarity with coding jargon. If you haven't heard (and how could you, it's only very recently that it started), there's a trend now where the power of these agents is inspiring plumbers to open up their terminals, parents and grandparents to google "how to install npm". On the other hand, the bulk of users are probably fairly computer-literate.
...
Call `prepare_skill_edit(slug=...)`. You do not have the installed skill's
path — it is not in your session's skills — and `/tmp` is not a valid staging
location here. It copies the library's current version into
`./.skill-staging/{slug}/` and marks the draft as derived from that skill,
which is what makes the user's save its next VERSION rather than a name
collision.

**Preserve the original name.** Keep the directory name and the `name`
frontmatter field unchanged: editing `research-helper` produces
`research-helper`, never `research-helper-v2`.
...
- Call `submit_skill` so the user can save it into their library.

Please add steps to your TodoList, if you have such a thing, so you don't forget — in particular "Create evals JSON and run `eval-viewer/generate_review.py` so the human can review test cases".
