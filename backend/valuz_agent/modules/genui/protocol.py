...
_ENUM_DESCRIPTION = re.compile(r"[\w.+\-]+(?:\s*\|\s*[\w.+\-]+)+")
...
def _parse_param_specs(params_doc: str) -> tuple[tuple[str, ...], dict[str, dict[str, Any]]]:
    ...
    # The grammar is ``{name: description, other?: description}`` and editions
    # may append free prose after the closing brace (``}; latest snapshots``).
    # Only the braced block carries parameters; a leading ``{`` left on the
    # first name or a trailing ``}`` glued to the last description would turn
    # ``symbol`` into ``{symbol`` and ``2-3650`` into a string.
    ...
    for raw_field in inner.split(","):
        ...
        if description == "boolean":
            ...
            elif _ENUM_DESCRIPTION.fullmatch(description):
                # Only a description that is nothing but choices is an enum;
                # prose such as ``exact prefixed stock|index|fund|crypto symbol``
                # documents a format and must not reject real values.
                spec["enum"] = tuple(
